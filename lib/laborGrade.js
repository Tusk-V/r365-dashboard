...
export const getDailyLaborGrade = (day, forecastVariance) => {
  ...
  if (forecastPct !== null && forecastPct < -5
    ...
    if (schOverPct > 5) {
      return { letter: 'F', color: 'text-white', bg: 'bg-red-600 border-red-500', score: 0, autoF: true };
    }
  }
  ...
  // --- Letter grade ---
  if (composite >= 90) return { letter: 'A', color: 'text-green-950', bg: 'bg-green-500 border-green-600', score: composite };
  if (composite >= 75) return { letter: 'B', color: 'text-white', bg: 'bg-blue-600 border-blue-500', score: composite };
  if (composite >= 60) return { letter: 'C', color: 'text-amber-950', bg: 'bg-amber-400 border-amber-500', score: composite };
  if (composite >= 40) return { letter: 'D', color: 'text-orange-950', bg: 'bg-orange-500 border-orange-600', score: composite };
  return { letter: 'F', color: 'text-white', bg: 'bg-red-600 border-red-500', score: composite };
};
