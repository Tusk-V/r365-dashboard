...
export default function Home() {
    ...
    try {
      ...
      data.values.forEach(row => {
        ...
        groupedByLocation[location].push({
          ...
          r365Forecast: (function() {
            var s = parseFloat(row[2]) || 0;
            var fv = parseFloat(row[5]) || 0;
  ...
  const loadForecastData = async () => {
    ...
    try {
      ...
      var parsed = data.values.map(function(row) {
        return {
          date: row[0] || '',
          location: row[1] || '',
  ...
  const getR365Forecast = (dateStr, locationName) => {
    ...
    const entry = locData.find(function(d) {
    ...
    return (entry && entry.r365Forecast) ? entry.r365Forecast : null;
  };
  ...
  const computeForecastForLocation = (locationName, weekOffset) => {
    const locData = forecastData.filter(function(d) { return d.location === locationName && d.sales > 0; });
    ...
    for (var i = 0; i < 7; i++) {
      ...
      const targetWeather = forecastData.find(function(d) { return d.location === locationName && d.date === targetKey; });
      const highTemp = targetWeather ? targetWeather.highTemp : null;
      const conditions = targetWeather ? (targetWeather.conditions || '') : '';
      const actualEntry = locData.find(function(d) { return d.date === targetKey; });
      const actualSales = actualEntry ? actualEntry.sales : null;
      ...
      const pwEntry = locData.find(function(d) { return d.date === pwKey; });
      const pwSales = pwEntry ? pwEntry.sales : null;
      const pwWeatherEntry = forecastData.find(function(d) { return d.location === locationName && d.date === pwKey; });
      const pwTemp = pwWeatherEntry ? pwWeatherEntry.highTemp : null;
      const pwConditions = pwWeatherEntry ? (pwWeatherEntry.conditions || '') : '';
      ...
      for (var w = 1; w <= 6; w++) {
        ...
        const pastEntry = locData.find(function(d) { return d.date === pastKey; });
      ...
      if (weeklyData.length >= 4) {
        ...
        weightedAvg = weeklyData.reduce(function(a, b) { return a + b; }, 0) / weeklyData.length;
      }

      const pyDate = new Date(targetDate);
      pyDate.setFullYear(pyDate.getFullYear() - 1);
      const pyKey = formatForecastDate(pyDate);
      const pyEntry = locData.find(function(d) { return d.date === pyKey; });
      const pySales = pyEntry ? pyEntry.sales : null;
      const pyWeather = pyEntry ? pyEntry.highTemp : null;
      const pyConditions = pyEntry ? (pyEntry.conditions || '') : '';

      var forecast = 0;
      var forecastMethod = 'none';
      var weatherAdj = 0;
      var pwOutlier = false;
      var useBaseline = pwSales;
      if (pwSales && pwSales > 0 && weightedAvg > 0) {
        var pwDeviation = Math.abs(pwSales - weightedAvg) / weightedAvg;
        if (pwDeviation > 0.30) {
      ...
      if (pwSales && pwSales > 0) {
        ...
        if (highTemp !== null && pwTemp !== null) {
          var tempDiffCalc = highTemp - pwTemp;
          if (tempDiffCalc >= 15) weatherAdj = 0.199;
          else if (tempDiffCalc >= 10) weatherAdj = 0.087;
          else if (tempDiffCalc >= 5) weatherAdj = 0.024;
          else if (tempDiffCalc >= 3) weatherAdj = 0.007;
          else if (tempDiffCalc <= -15) weatherAdj = -0.151;
          else if (tempDiffCalc <= -10) weatherAdj = -0.059;
          else if (tempDiffCalc <= -5) weatherAdj = -0.043;
          else if (tempDiffCalc <= -3) weatherAdj = -0.020;
          else weatherAdj = 0.040;
        }
        var thisRain = conditions.toLowerCase().includes('rain') || conditions.toLowerCase().includes('storm') || conditions.toLowerCase().includes('shower');
        var pwRainCheck = pwConditions.toLowerCase().includes('rain') || pwConditions.toLowerCase().includes('storm') || pwConditions.toLowerCase().includes('shower');
        if (thisRain && !pwRainCheck) weatherAdj -= 0.133;
        else if (!thisRain && pwRainCheck) weatherAdj += 0.133;
        forecast = useBaseline * (1 + weatherAdj);
      } else if (weightedAvg > 0) {
        forecastMethod = 'avg';
        if (highTemp !== null) {
          var idealTemp = 75;
          var absTempDiff = highTemp - idealTemp;
          if (absTempDiff < -20) weatherAdj = -0.12;
          else if (absTempDiff < -10) weatherAdj = -0.06;
          else if (absTempDiff < 0) weatherAdj = -0.02;
          else if (absTempDiff <= 10) weatherAdj = 0.04;
      ...
      if (pwConditions && conditions && pwConditions !== conditions) {
        var shortCond = function(cc) {
          if (cc.toLowerCase().includes('rain')) return 'Rain';
          if (cc.toLowerCase().includes('overcast')) return 'Overcast';
          if (cc.toLowerCase().includes('partly')) return 'Ptly Cloudy';
          if (cc.toLowerCase().includes('clear') || cc.toLowerCase().includes('sunny')) return 'Clear';
          if (cc.toLowerCase().includes('cloud')) return 'Cloudy';
          return cc.split(',')[0].substring(0, 12);
        };
        var fromC = shortCond(pwConditions);
        var toC = shortCond(conditions);
        if (fromC !== toC) conditionChange = fromC + ' \u2192 ' + toC;
      }

      var month = targetDate.getMonth() + 1;
      var dateNum = targetDate.getDate();
      var holiday = null;
      if (month === 2 && dateNum === 14) holiday = "Valentine's Day";
      if (month === 7 && dateNum === 4) holiday = "4th of July";
      if (month === 12 && dateNum === 25) holiday = "Christmas";
      if (month === 12 && dateNum === 24) holiday = "Christmas Eve";
      if (month === 10 && dateNum === 31) holiday = "Halloween";

      var isToday = targetKey === formatForecastDate(new Date());
      var isPast = targetDate < new Date() && !isToday;

      result.push({
        dayLabel: dayNames[i] + ' ' + (targetDate.getMonth() + 1) + '/' + targetDate.getDate(),
        date: targetKey,
        forecast: Math.round(forecast),
        r365Forecast: r365Forecast,
        ...
        pyWeather: pyWeather,
        pyConditions: pyConditions,
        tempDelta: tempDelta,
        tempCompare: tempCompare,
        ...
        <div className="max-w-[1400px] mx-auto">
          ...
          {activeTab === 'forecast' && (
            <>
              <div className="bg-slate-800 border border-slate-700 rounded-lg p-3 mb-3 shadow-lg">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <div className="flex bg-slate-700 rounded-lg overflow-hidden">
                    {[
                      ...
                      return (
                        <button
                          key={w.val}
                          onClick={function() { setForecastWeekOffset(w.val); }}
                  ...
                  <select
                    ...
                    onChange={function(e) { setForecastMarketFilter(e.target.value); }}
                ...
                <div className="text-xs text-slate-500 mt-1.5">
                  {(function() {
                    var mon = getWeekMonday(forecastWeekOffset);
                    var sun = new Date(mon);
                    sun.setDate(mon.getDate() + 6);
                    var label = forecastWeekOffset === -1 ? 'Last Week' : forecastWeekOffset === 0 ? 'This Week' : 'Next Week';
              ...
              {!forecastLoading && forecastData.length > 0 && (function() {
                var allLocs = [...new Set(forecastData.map(function(d) { return d.location; }))].sort();
                var filteredLocs = forecastMarketFilter === 'all' ? allLocs : allLocs.filter(function(l) { return getMarket(l) === forecastMarketFilter; });
                var accessLocs = isAdmin ? filteredLocs :
                  dashboardAccess && dashboardAccess.type === 'specific' ? filteredLocs.filter(function(l) { return dashboardAccess.locations && dashboardAccess.locations.includes(l); }) :
                  dashboardAccess && dashboardAccess.type === 'all' ? filteredLocs : [];

                var locAccuracy = accessLocs.map(function(loc) {
                  var weekResults = [];
                  for (var w = -1; w >= -4; w--) {
                    var days = computeForecastForLocation(loc, w);
                    var withBoth = days.filter(function(d) { return d.forecast > 0 && d.actual !== null && d.actual > 0; });
                    if (withBoth.length === 0) continue;
                    var totalF = withBoth.reduce(function(s, d) { return s + d.forecast; }, 0);
                    var totalA = withBoth.reduce(function(s, d) { return s + d.actual; }, 0);
                    var acc = totalA > 0 ? (1 - Math.abs(totalF - totalA) / totalA) * 100 : 0;
                    var r365WithBoth = withBoth.filter(function(d) { return d.r365Forecast !== null && d.r365Forecast > 0; });
                    var r365TotalF = r365WithBoth.reduce(function(s, d) { return s + d.r365Forecast; }, 0);
                    var r365TotalA = r365WithBoth.reduce(function(s, d) { return s + d.actual; }, 0);
                    var r365Acc = r365TotalA > 0 && r365TotalF > 0 ? (1 - Math.abs(r365TotalF - r365TotalA) / r365TotalA) * 100 : null;
                    var mon = getWeekMonday(w);
                    weekResults.push({ week: w, accuracy: Math.round(acc * 10) / 10, r365Accuracy: r365Acc !== null ? Math.round(r365Acc * 10) / 10 : null, label: (mon.getMonth()+1) + '/' + mon.getDate() });
                  }
                  if (weekResults.length === 0) return null;
                  var avgAccuracy = weekResults.reduce(function(s, wr) { return s + wr.accuracy; }, 0) / weekResults.length;
                  var r365Weeks = weekResults.filter(function(wr) { return wr.r365Accuracy !== null; });
                  var avgR365 = r365Weeks.length > 0 ? r365Weeks.reduce(function(s, wr) { return s + wr.r365Accuracy; }, 0) / r365Weeks.length : null;
                  var trend = 'flat';
                  if (weekResults.length >= 3) {
                    var recent = weekResults.slice(0, Math.ceil(weekResults.length / 2));
                    var older = weekResults.slice(Math.ceil(weekResults.length / 2));
                    var recentAvg = recent.reduce(function(s, wr) { return s + wr.accuracy; }, 0) / recent.length;
                    var olderAvg = older.reduce(function(s, wr) { return s + wr.accuracy; }, 0) / older.length;
                    if (recentAvg - olderAvg > 2) trend = 'improving';
                    else if (olderAvg - recentAvg > 2) trend = 'declining';
                  }
                  return { location: loc, weeks: weekResults, avgAccuracy: Math.round(avgAccuracy * 10) / 10, avgR365: avgR365 !== null ? Math.round(avgR365 * 10) / 10 : null, trend: trend };
                }).filter(Boolean);

                if (locAccuracy.length === 0) return null;
                var overallAvg = locAccuracy.reduce(function(s, l) { return s + l.avgAccuracy; }, 0) / locAccuracy.length;
                var r365Locs = locAccuracy.filter(function(l) { return l.avgR365 !== null; });
                var overallR365 = r365Locs.length > 0 ? r365Locs.reduce(function(s, l) { return s + l.avgR365; }, 0) / r365Locs.length : null;

                return (
                  <div className="bg-slate-800 border border-slate-700 rounded-lg mb-3 overflow-hidden shadow-lg">
                    <div className="px-4 py-2.5 border-b border-slate-700 flex items-center justify-between cursor-pointer select-none" onClick={function() { setForecastAccuracyExpanded(!forecastAccuracyExpanded); }}>
                      <div className="flex items-center gap-2">
                        <ChevronDown className={'w-4 h-4 text-slate-400 transition-transform ' + (forecastAccuracyExpanded ? '' : '-rotate-90')} />
                        <div>
                          <div className="text-sm font-semibold text-white">Forecast Accuracy &mdash; Rolling 4 Weeks</div>
                          <div className="text-xs text-slate-400">Model vs R365 accuracy comparison</div>
                        </div>
                      </div>
                      ...
                      <table className="w-full text-xs" style={{ fontSize: '0.78rem' }}>
                        <thead>
                          <tr className="text-slate-400 uppercase" style={{ fontSize: '0.68rem' }}>
                            ...
                            {locAccuracy[0] && locAccuracy[0].weeks.map(function(wr, i) {
                              return <th key={i} className="text-right px-2 py-1.5 font-semibold">Wk {wr.label}</th>;
                            })}
                        ...
                        <tbody>
                          {locAccuracy.sort(function(a, b) { return b.avgAccuracy - a.avgAccuracy; }).map(function(loc) {
                            return (
                              <tr key={loc.location} className="border-b border-slate-700/40">
                                ...
                                {loc.weeks.map(function(wr, i) {
                                  return (
                                    <td key={i} className={'text-right px-2 py-1.5 font-semibold ' + (wr.accuracy >= 96 ? 'text-green-400' : wr.accuracy >= 92 ? 'text-yellow-400' : wr.accuracy >= 85 ? 'text-orange-400' : 'text-red-400')}>
                                      {wr.accuracy}%
                                    </td>
                                  );
                                })}
                                ...
                                <td className="text-right pr-3 py-1.5">
                                  {loc.trend === 'improving' ? <span className="text-green-400">&uarr;</span> : loc.trend === 'declining' ? <span className="text-red-400">&darr;</span> : <span className="text-slate-500">&rarr;</span>}
                                </td>
                              </tr>
                            );
                          })}
              ...
              {!forecastLoading && forecastData.length > 0 && (function() {
                ...
                return accessLocs.map(function(loc) {
                  var days = computeForecastForLocation(loc, forecastWeekOffset);
                  if (days.length === 0) return null;
                  var totalForecast = days.reduce(function(s, d) { return s + d.forecast; }, 0);
                  var totalR365 = days.filter(function(d) { return d.r365Forecast !== null; }).reduce(function(s, d) { return s + d.r365Forecast; }, 0);
                  var totalActual = days.filter(function(d) { return d.actual !== null; }).reduce(function(s, d) { return s + d.actual; }, 0);
                  var totalAvg = days.reduce(function(s, d) { return s + d.weightedAvg; }, 0);
                  var totalPW = days.filter(function(d) { return d.pwSales !== null; }).reduce(function(s, d) { return s + d.pwSales; }, 0);
                  var totalPY = days.filter(function(d) { return d.pySales !== null; }).reduce(function(s, d) { return s + d.pySales; }, 0);
                            ...
                            {days.map(function(day, idx) {
                              return (
                                <tr key={idx} className={'border-b border-slate-700/40 ' + (day.isToday ? 'bg-blue-900/10' : '')}>
                                  ...
                                  <td className="text-right px-1.5 py-1.5">
                                    <div className="relative inline-block group cursor-default">
                                      ...
                                      <div className="hidden group-hover:block absolute bottom-full right-0 mb-1.5 bg-slate-900 border border-slate-600 rounded-md p-2 min-w-[180px] z-50 shadow-xl whitespace-normal">
                                        ...
                                        {day.pwOutlier && <div className="text-[9px] text-amber-400 py-px">&#9888; PW was &gt;30% off avg &mdash; blended 60/40</div>}
                                  ...
                                  <td className="text-right pl-1.5 pr-1 py-1.5 text-slate-400 border-l border-slate-600">{day.pwSales !== null ? '$' + day.pwSales.toLocaleString() : <span className="text-slate-600">&mdash;</span>}</td>
                                  ...
                                  <td className="text-right pl-1.5 pr-1 py-1.5 text-slate-400 border-l border-slate-600">{day.pySales !== null ? '$' + day.pySales.toLocaleString() : <span className="text-slate-600">&mdash;</span>}</td>
